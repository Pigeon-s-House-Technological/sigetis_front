...
const HomeEvaluacionEnPares = () => {
  ...
  const fetchAutoevaluaciones = async () => {
    try {
      ...
      const autoevaluacionesTipo1 = response.data.filter(evaluacion => evaluacion.tipo_evaluacion === 3);
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post(`${API_BASE_URL}/evaluaciones`, {
        ...
        tipo_evaluacion: 3, // Asegúrate de que el tipo de evaluación es 1
  ...
  return (
    <div className='HomeAutoevaluacion'>
      <div className='row-home'>
        <h2 className='col col-h1'>Evaluaciones en Pares</h2>
        <div className='col col-h3'>
          <h4>{autoevaluaciones.length} evaluaciones en pares</h4>
        </div>
        <div className='col col-button'>
          <Button style={{backgroundColor: '#215f88'}} className="btn-custom-primary" onClick={handleClick}>Agregar Evaluación en Pares</Button>
        </div>
      </div>
      ...
      <Modal show={showModal} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Agregar Evaluación en Pares</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
      ...
      <Modal show={showConfirmModal} onHide={handleCloseConfirmModal}>
        ...
        <Modal.Body>
          ¿Estás seguro de que deseas eliminar esta evaluación en pares?
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showEditModal} onHide={handleCloseEditModal}>
        <Modal.Header closeButton>
          <Modal.Title>Editar Evaluación en Pares</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
...
export default HomeEvaluacionEnPares;